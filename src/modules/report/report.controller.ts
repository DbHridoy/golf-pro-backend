...
class ReportController {
  createReport = asyncHandler(async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      if (!data.contentType || !["profile", "post"].includes(data.contentType)) {
      ...
      if (
        ...
        res.status(400).json({
          message: "reportedUserId or reportedClubId is required for profile reports",
        });
      }

      if (data.contentType === "post" && !data.reportedPostId) {
        res.status(400).json({ message: "reportedPostId is required for post reports" });
      }
