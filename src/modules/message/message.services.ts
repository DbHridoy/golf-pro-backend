...
import type { NewMessagePayload } from "./message.interface";
...
// send message
async function new_message_IntoDb(user: JwtPayload, data: NewMessagePayload) {
...
// update message
async function updateMessageById_IntoDb(messageId: string, updateData: Partial<{ text: string; imageUrl: string[] }>) {
...
async function deleteMessageById_IntoDb(messageId: string) {
  const session = await mongoose.startSession();
  session.startTransaction();
